...
- **AI calls**: `ChefService.askChefHarris()` (`lib/services/chef_service.dart`)
  is the single centralized AI call for recipe generation and chat. Calls the
...
4. **Fridge notification replacing the Fridge tab — new, per `docs/decisions_2026-08-17.md` item 5.** A single local scheduled notification, fired 2 days after unused Fridge Clearer ingredients, one nudge only, never repeated, two CTAs (Fridge Clearer, AI generator). Originally scoped as the fix for Fridge Countdown's cold-start dead end; superseded by the sheet's removal below. Not started. **`FridgeCountdownSheet`/`lib/widgets/fridge_countdown_sheet.dart` was deleted as dead code (2026-08-18) — confirmed unreachable from any live navigation path once the Fridge tab (its only entry point, the "N items expiring" chip) was cut.** `FridgeCountdownService` and the `fridge_items` table it reads/writes are now orphaned too (no other caller) but were left in place — removing them touches `CookModeSurface.fridgeCountdown`/`ChefRecipeSurface.fridgeCountdown` ledger-eligibility plumbing and a DB CHECK constraint permitting historical `source='fridge_countdown'` rows, a bigger call than the sheet deletion itself. If this notification item is picked back up, design it as a fresh entry point into Fridge Clearer/the AI generator, not a revival of the old sheet.
5. **Waste Ledger verdicts + permanent ledger explainer — new, per `docs/decisions_2026-08-17.md` item 6.** Every completed cook should get a verdict explaining why it did or did not count toward the ledger; plus a permanent explainer on the ledger screen itself. Rationale on record: correct behavior currently reads as inconsistent because nothing in the UI explains the rules. Not started.
...
7. **Waste Ledger write-failure recovery — schema/service layer built 2026-08-16, not wired to anything user-facing.** `PendingLedgerWriteService` + `LedgerService`'s sealed `LedgerCompletionSuccess`/`LedgerCompletionWriteFailed` result + `retryPendingWrite` all exist but nothing calls `retryPendingWrite` yet — no persistent affordance UI, no retry trigger, no automatic scheduling. Also still untested: an airplane-mode test of the full post-cook sequence surviving a failed ledger write was planned but never run. See `docs/CHANGELOG.md` for the full design/implementation record.
8. **`LedgerService.freshProduceOnly` blocklist — product question, not a bug.** Confirmed live: potatoes (and other long-life-pantry-staple-keyword matches) don't count as "rescued" even when genuinely at risk of spoiling. Per `docs/decisions_2026-08-17.md`, the intended real fix is provenance-based. Note: the `CookModeSurface.fridgeCountdown`/`fridge_items` provenance data this idea originally pointed at is now stale/orphaned — `FridgeCountdownSheet`, its only writer, was deleted as dead code (2026-08-18, see Roadmap item 4) — so this fix would need a live provenance source, not the old one. Not started.
...
15. **OpenAI prompt caching — priority raised 2026-08-17, do not treat as closed.** Previously investigated and closed 2026-08-15 as "real but small money on the table" (~$0.0022/call estimated saving) at the volume assumed then. The ~7,095-char static system prompt is already a clean, correctly-positioned cache-eligible prefix, sent identically on every call including SOS — no prompt restructuring needed to enable caching, only instrumentation to confirm it's actually engaging (`prompt_tokens_details.cached_tokens` is not currently captured anywhere). Re-evaluate now that priority has been explicitly raised.
