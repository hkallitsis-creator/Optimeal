...
4. **Fridge notification replacing the Fridge tab — new, per `docs/decisions_2026-08-17.md` item 5.** A single local scheduled notification, fired 2 days after unused Fridge Clearer ingredients, one nudge only, never repeated, two CTAs (Fridge Clearer, AI generator). This is the chosen fix for Fridge Countdown's cold-start dead end (a user with zero/only-fresh items currently can never open the Fridge Countdown sheet at all, since its only entry point is the "N items expiring" chip) — replaces the earlier idea of a persistent Home entry point. Not started. Fridge Countdown's underlying "Use Tonight" action is also not gated on `isExpiringSoon` (usable on anything regardless of freshness) and doesn't feed/read `RecentGenerationsService` for variety — both still open, low priority, worth revisiting alongside this.
