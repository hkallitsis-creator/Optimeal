...
- **AI calls**: `ChefService.askChefHarris()` (`lib/services/chef_service.dart`)
  ...
  `api_call_cost_log.surface`. **`kChefCallSurfaces` holds six values since
  ...
  They are not separate call sites — the same two sites send a different
  `surface` on a retry. Message assembly lives in the separately
